...
from .upgrade_mitigation import measurement_probs,groups,estimated_vector,LINEAR
from .upgrade_detection import predict
CFG=json.loads((ROOT/'results/stage4_upgrade_v1/detection/frozen.json').read_text())
...
def distributions(row,p,fold=1,sv=False):
 n=row['n'];g=row['gates'];g=[gate for x in g for gate in ([x]*fold if x[0]=='CNOT' else [x])];key=dict(n=n,p=p,gates=g,backend=sha(ROOT/'annni/upgrade_gates.py'),measure=sha(ROOT/'annni/upgrade_mitigation.py'),sv=sv);uid=hashlib.sha256(json.dumps(key,sort_keys=True).encode()).hexdigest();path=OUT/'end_to_end/probabilities'/uid;path.parent.mkdir(exist_ok=True);meta=path.with_suffix('.json');archive=path.with_suffix('.npz')
 if meta.exists():
 ...
 guard();t=time.perf_counter();rho=density(g,n,p);assert abs(np.trace(rho)-1)<1e-9 and np.max(abs(rho-rho.conj().T))<1e-10
 if p==0:np.testing.assert_allclose(rho,np.outer(row['state'],row['state'].conj()),atol=1e-10)
 dist=groups(rho) if sv else {b:measurement_probs(rho,b) for b in ['Z'*n,'X'*n]};ob=vector(observations(rho,n));np.savez_compressed(archive,**{f'p{i}':v for i,v in enumerate(dist.values())},observables=ob);r=dict(key=json.loads(json.dumps(key)),bases=list(dist),sha256=sha(archive),archive=str(archive.relative_to(ROOT)),seconds=time.perf_counter()-t,cnots=resources(g,n)['cnots'],disposition='newly_executed');dump(meta,r);return dist,r
...
def measured(row,p,method):
 n=row['n'];key=dict(kappa=row['kappa'],h=row['h'],gates=row['gates'],p=p,method=method,n=n,code=sha(ROOT/'annni/stage5_e2e.py'),diagnostic_sha=sha(ROOT/'results/stage4_upgrade_v1/detection/frozen.json'),budgets=PLAN['mitigation_shots'],replicates=32);uid=hashlib.sha256(json.dumps(key,sort_keys=True).encode()).hexdigest();path=OUT/'end_to_end/measurements'/uid;path.parent.mkdir(exist_ok=True);meta=path.with_suffix('.json');archive=path.with_suffix('.npz')
