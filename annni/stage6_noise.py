"""Literal per-CNOT density/measurement pipeline, independent of any detector."""
import resource
...
def measured(row,p,method,budgets=None):
 ...
 for budget in budgets:
  rng=np.random.default_rng(np.random.SeedSequence([PLAN['measurement']['seed'],*np.frombuffer(bytes.fromhex(uid(key)),dtype='<u4').tolist(),budget]));scale_amounts=[budget] if len(folds)==1 else [budget//3+(i<budget%3) for i in range(3)];group_list=[]
  ...
  for rep in range(32):
   vs=[];cc=[];dd=[]
   for d,(bs,alloc) in zip(ds,group_list):
    c=np.array([rng.multinomial(int(a),d[b]) for b,a in zip(bs,alloc)],dtype=np.int32);v,den=estimated_vector({b:x/int(a) for b,x,a in zip(bs,c,alloc)},n,method=='sv');vs.append(v);cc.extend(c);dd.append(den)
 ...
 base.parent.mkdir(exist_ok=True);a=base.with_suffix('.npz');np.savez_compressed(a,exact=truth,own_clean=vector(observations(row['state'],n)),**data);result=dict(key=key,n=n,kappa=row['kappa'],h=row['h'],p=p,method=method,archive=str(a.relative_to(ROOT)),sha256=sha(a),probability_records=records,sampling=sampling,cost=cost,quality=quality,seconds=time.perf_counter()-t,disposition='newly_executed',noise_scope='Every CNOT including references/decoding/folding target-depolarized',randomization='Independent full-hash-key streams per physical estimator; same archived counts reused across detectors; correlated observables within each joint bitstring',unclipped=True);dump(meta,result);return result
