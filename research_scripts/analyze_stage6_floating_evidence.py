"""Finite-size evidence map with distinct stopping, bond, initial-state and size checks."""
import sys,csv
...
for task in ['coarse','bounded','followup','controls']:
...
for h in sorted(set(r['h'] for r in allrows)):
 ...
 rows.append(dict(kappa=.8,h=h,status=status,all_required_controls_present=complete,all_required_solver_stops=bool(solver),strict_bond_and_initial_agreement=bool(strict),N96_power_q_consistency=size_compatible,larger_size_contradiction=bool(larger_contradiction),initial_agreement=ia,bond_agreement=ba,metrics=detail,largeN_extra=[x for x in detail if x['n']>=160],interpretation='Discrete tested evidence, OBC only; no transfer of labels to N8 PBC. Fit comparisons are finite-window predictive checks, not rigorous statistical p-values.'))
 dump(O/'evidence_map.json',rows)
...
with (O/'evidence_map.csv').open('w') as file:w=csv.DictWriter(file,fieldnames=flat[0]);w.writeheader();w.writerows(flat)
print(summary)
