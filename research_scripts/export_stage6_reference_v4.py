"""Version4 layered reference with cross-size Friedel conflicts: physical labels and RN proxies remain separate."""
import sys,csv
...
p=OUT/'floating_boundary_scan/evidence_map_v2.json'
if p.exists():
...
dump(O/'reference_regions_v4.json',rows)
with (O/'reference_regions_v4.csv').open('w') as f:w=csv.DictWriter(f,fieldnames=sorted({key for r in rows for key in r}));w.writeheader();w.writerows(rows)
boundaries=json.loads((O/'boundary_reference.json').read_text());p=OUT/'floating_boundary_scan/interval_evidence_v2.json';interval=json.loads(p.read_text()) if p.exists() else {}
extra=[]
...
dump(O/'boundary_reference_v4.json',boundaries+extra)
with (O/'boundary_reference_v4.csv').open('w') as f:w=csv.DictWriter(f,fieldnames=boundaries[0]);w.writeheader();w.writerows(boundaries+extra)
print('Reference v4 regions',len(rows),'RN features',len(boundaries),'requested OBC transition intervals',len(extra))
