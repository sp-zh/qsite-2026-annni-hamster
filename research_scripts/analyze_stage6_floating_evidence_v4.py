"""Evidence v4: unchanged criteria, adding the registered N160 double-initial size check.
...
for h in sorted(set(r['h'] for r in allrows)):
 ...
 dump(O/'evidence_map_v4.json',rows)
...
summary=dict(version=4,timestamp=datetime.now(timezone.utc).isoformat(),prior_evidence_sha=sha(O/'evidence_map_v2.json'),continuation_registration_sha=sha(O/'final_continuation_registration.json'),size_registration_sha=sha(O/'h425_N160_registration.json'),protocol_sha=sha(O/'evidence_protocol_v1.json'),analysis_sha=sha(__file__),supported_samples=supported,lower_transition_bracket=lower,upper_transition_bracket=upper,candidate_floating_samples=[r['h'] for r in rows if r['status'].startswith('candidate_floating')],unresolved_samples=[r['h'] for r in rows if r['status']=='finite_size_crossover_unresolved'],scope='Brackets span supported sampled sides and may contain unresolved points; not exact thermodynamic phase boundaries or confidence intervals.',raw_observable_antiphase_check='Ordered phase can show entropy constant offsets from near-degenerate branches; full strict entropy agreement is reported separately even when order/q/c across both initial states support antiphase physics.');dump(O/'interval_evidence_v4.json',summary)
...
with (O/'evidence_map_v4.csv').open('w') as file:w=csv.DictWriter(file,fieldnames=flat[0]);w.writeheader();w.writerows(flat)
print(summary)
