...
O=OUT/'map_shard_high_k';O.mkdir(exist_ok=True);rows=[];start=time.perf_counter()
...
dump(O/'index.json',rows);dump(O/'metadata.json',dict(config_sha256=sha(OUT/'experiment_plan.json'),source_fingerprint=fp(),scope='kappa>=.8 map points, identical frozen protocol',elapsed_seconds=time.perf_counter()-start,cache_disposition='new computations; reused by main map runner',no_shared_index_mutation=True))
