...
p=argparse.ArgumentParser();p.add_argument('--task',choices=['core','full_best','n12'],default='core');args=p.parse_args();O=OUT/'end_to_end';rows=[];start=time.perf_counter();cfg=json.loads((OUT/'selector_frozen.json').read_text())['selector'];historical=json.loads((ROOT/'results/stage4_upgrade_v1/map/index.json').read_text());oldhva=json.loads((ROOT/'results/stage3_v1/grid/selection_frozen.json').read_text())['rows']
coords=PLAN['end_to_end'] if args.task=='core' else PLAN['final_map'] if args.task=='full_best' else PLAN['n12_mitigation_points']
...
for ii,(k,h) in enumerate(coords):
 ...
 for noise in [0,.01,.05]:
  combos=[('B0','raw'),('B3','raw'),('B5','raw'),('B5','zne'),('B5','sv')] if args.task=='core' else [('B5',json.loads((O/'mitigation_frozen.json').read_text())['selected'])] if args.task=='full_best' else [('B5','raw'),('B5','sv')]
