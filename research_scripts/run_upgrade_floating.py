...
R=Path(__file__).resolve().parents[1];O=R/'results/stage4_upgrade_v1/floating';O.mkdir(exist_ok=True);PLAN=json.loads((O.parent/'experiment_plan.json').read_text())
def dump(p,x):
...
def run(n,k,h,chi,init='plus',max_sweeps=20):
...
def verify():
 ...
 for n in [8,12]:
  ...
  del a,expected,ex
  row=run(n,.8,.5,64,max_sweeps=20);e,s=eigsh(ed_matrix(n,.8,.5),k=1,which='SA',tol=1e-12);ep,sp=eigsh(ed_matrix(n,.8,.5,True),k=1,which='SA',tol=1e-12)
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--task',choices=['verify','coarse','refine'],required=True);a=p.parse_args()
 if a.task=='verify':verify()
 elif a.task=='coarse':
  rows=[]
  for k,h in PLAN['floating']['controls']:rows.append(run(32,k,h,64))
  for k in PLAN['floating']['kappa']:
   for h in PLAN['floating']['h']:rows.append(run(32,k,h,64));dump(O/'coarse_index.json',rows)
 elif a.task=='refine':
  sel=json.loads((O/'candidate_windows.json').read_text());rows=[]
  for k,h in sel['points']:
   for n in [32,64,96]:
    for chi in [64,128]:rows.append(run(n,k,h,chi));dump(O/'refine_index.json',rows)
  for k,h in sel['centers']:
   for n in [64,96]:
    for init in ['plus','antiphase']:rows.append(run(n,k,h,256,init));dump(O/'refine_index.json',rows)
