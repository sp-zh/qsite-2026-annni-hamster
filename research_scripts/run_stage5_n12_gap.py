...
p=argparse.ArgumentParser();p.add_argument('--task',choices=['audit','validation','test','n12test','n12_192','n12_192_gap','windows','map'],required=True);args=p.parse_args();task=args.task
...
if task in ['audit','map']:
 ...
 coords=PLAN['validation_v2'] if task=='validation' else PLAN['test_v2'] if task=='test' else PLAN['n12_test_v2'] if task=='n12test' else PLAN['slices']+PLAN['n12_test_v2'] if task in ['n12_192','n12_192_gap'] else [(k,h) for k,hs in PLAN['windows'] for h in hs];work=[(task,dict(kappa=k,h=h,n=12 if task.startswith('n12') else 8)) for k,h in coords]
if task=='n12_192_gap':work=work[66:72]
...
for ii,(group,point) in enumerate(work):
 ...
 else:runs=[adaptive(n,k,h,ref,11,budget=192 if task in ['n12_192','n12_192_gap'] else 128,group=task) for ref in PLAN['refs']]
