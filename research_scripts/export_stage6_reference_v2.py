...
for task in ['development','validation','confirmation']:
 ...
 for r in json.loads(p.read_text()):rows.append(dict(cohort=task,n='8;12;16',bc='PBC',kappa=r['kappa'],h=r['h'],**scopes(r['label'],r['level']),sources=';'.join(r['sources']),evidence=r['evidence']))
...
dump(O/'reference_regions_v2.json',rows)
with (O/'reference_regions_v2.csv').open('w') as f:w=csv.DictWriter(f,fieldnames=sorted({key for r in rows for key in r}));w.writeheader();w.writerows(rows)
...
dump(O/'boundary_reference_v2.json',boundaries+extra)
with (O/'boundary_reference_v2.csv').open('w') as f:w=csv.DictWriter(f,fieldnames=boundaries[0]);w.writeheader();w.writerows(boundaries+extra)
print('Reference v2 regions',len(rows),'RN features',len(boundaries),'requested OBC transition intervals',len(extra))
