...
O=OUT/'end_to_end';index=json.loads((OUT/'selector_benchmark/windows/index.json').read_text())+json.loads((OUT/'selector_benchmark/refined/index.json').read_text());curves=[];peaks=[]
 ...
 for arm in ['B3','B5']:
  for p in [0,.01,.05]:
   ...
   for name,y in [('D1_change',dv),('D2_chi',chi)]:
    ...
    for i in indices:peaks.append(dict(kappa=k,arm=arm,p=p,metric=name,support=[float(hs[i]),float(hs[i+1])],midpoint=float(mid[i]),height=float(y[i]),endpoint=i in [0,len(y)-1],quality_support=bool(peak_support_mask(quality)[i]),structure_switch=switch[i],smoothing=None,step=.0125))
   dump(O/'state_access_curves_refined.json',curves);dump(O/'state_access_peaks_refined.json',peaks);print(k,arm,p,flush=True)
