...
row=dict(time=now.isoformat(),deadline=PLAN['deadline'],elapsed_seconds=(now-datetime.fromisoformat(PLAN['started_utc'])).total_seconds(),progress=records,process_snapshot=active,remaining_seconds=(datetime.fromisoformat(PLAN['deadline'])-now).total_seconds(),resource_policy='At most two heavy workers; BLAS/OMP one thread. 6GiB working-memory budget, not OS-enforced.',continuation='See each progress record exact next_command; queues are active only while this agent session remains running.')
