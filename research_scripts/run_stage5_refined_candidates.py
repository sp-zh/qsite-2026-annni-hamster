...
p=argparse.ArgumentParser();p.add_argument('--task',choices=['audit','validation','test','n12test','n12_192','windows','refined','map'],required=True);args=p.parse_args();task=args.task
...
if task in ['audit','map']:
 ...
 coords=PLAN['validation_v2'] if task=='validation' else PLAN['test_v2'] if task=='test' else PLAN['n12_test_v2'] if task=='n12test' else PLAN['slices']+PLAN['n12_test_v2'] if task=='n12_192' else json.loads((OUT/'end_to_end/grid_refinement_frozen.json').read_text())['coordinates'] if task=='refined' else [(k,h) for k,hs in PLAN['windows'] for h in hs];work=[(task,dict(kappa=k,h=h,n=12 if task.startswith('n12') else 8)) for k,h in coords]
