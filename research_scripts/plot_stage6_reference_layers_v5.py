...
paths=[A/'reference_regions_v5.csv',A/'boundary_reference_v5.csv',OUT/'floating_boundary_scan/evidence_map_v3.csv']
...
axs[2].set(yticks=range(5),yticklabels=[x[1] for x in categories],xlabel='h at kappa=0.8',title='Large-N OBC evidence v3: discrete samples',ylim=(-.5,4.5),xlim=(.075,.825));axs[2].invert_yaxis()
...
fig.savefig(F/'reference_layers_v5.png',dpi=170);plt.close(fig)
dump(F/'reference_layers_v5_sources.json',dict(sources=[dict(path=str(p.relative_to(ROOT)),sha256=sha(p)) for p in paths],code_sha=sha(__file__),data=dict(regions=r,RN_features=b,OBC_evidence=m)))
print('Reference layers plotted from v5 atlas and v3 OBC evidence')
